...
export async function POST(request: NextRequest) {
    ...
    if (key && keyCheck(key)) {
        let conn = null;
        const query = `INSERT INTO project(name, start, end, introduce, stack, takeaway, image, isbackup) VALUES('${name}','${start}','${end}','${introduce}','${stack}','${takeaway}','${image}',0), ('${name}','${start}','${end}','${introduce}','${stack}','${takeaway}','${image}',1)`;
        try {
